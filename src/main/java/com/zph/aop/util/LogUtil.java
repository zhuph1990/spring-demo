...
public class LogUtil {
     ...
     * 2、匹配任意类型的参数，只能匹配一个
     * execution( public Integer com.zph.aop.service.M*Calculator.*(Integer,*))
     * 3、*在进行匹配的时候只能匹配一层路径，不能匹配多层
     * execution( public Integer com.zph.aop.*.*.M*Calculator*.*(Integer,*))
     * 4、*不能够匹配访问修饰符，如果不确定访问修饰符是什么，可以直接省略不写
     * execution( Integer com.zph.aop.service.MyCalculator.*(Integer,*))
     * 5、返回值可以使用*来代替
     *
     * ..:
     * 1、可以匹配多个参数，任意类型
     ...
     * 在使用表达式的时候还支持逻辑运算
     * &&:多个条件必须同时满足
     * execution(public Integer com.zph.aop.service.MyCalculator.*(..)) && execution(* *(..))
     * ||：多个条件只要满足其中一个即可
     * execution(public Integer com.zph.aop.service.MyCalculator.*(..)) || execution(* *(..))
     * !：取反,除了这种情况的其他都满足
     * !execution(public Integer com.zph.aop.service.MyCalculator.add(Integer,Integer))
     *
     ...
     * @Before(value = "execution(public Integer com.zph.aop.service.MyCalculator.*(Integer,Integer)) && args(joinPoint,k)",argNames = "joinPoint,k")
     *
    ...
    private static void start(JoinPoint joinPoint) {
        ...
        System.out.println("LOGGER---" + signature.getName() + "方法开始执行：参数是" + Arrays.asList(args));
    }

    @AfterReturning(value = "myPointCut()", returning = "result")
    public static void stop(JoinPoint joinPoint, Object result) {
        Signature signature = joinPoint.getSignature();
        System.out.println("LOGGER---" + signature.getName() + "方法执行结束，结果是：" + result);
    }

    @AfterThrowing(value = "myPointCut()", throwing = "e")
    public static void logException(JoinPoint joinPoint, Exception e) {
        Signature signature = joinPoint.getSignature();
        System.out.println("LOGGER---" + signature.getName() + "方法抛出异常：" + e.getMessage());
    }

    @After("myPointCut()")
    public static void logFinally(JoinPoint joinPoint) {
        Signature signature = joinPoint.getSignature();
        System.out.println("LOGGER---" + signature.getName() + "方法执行结束。。。。。over");
    ...
    public Object around(ProceedingJoinPoint pjp) throws Throwable {
        ...
        try {
            ...
            System.out.println("LOGGER---环绕异常通知：" + signature.getName() + "出现异常");
            throw throwable;
        } finally {
            System.out.println("LOGGER---环绕返回通知：" + signature.getName() + "方法返回结果是：" + result);
        }
        return result;
    }
}
